           ...
           <div key={index} className="flex items-center space-x-2 text-sm">
             ...
             <span className="font-medium text-popover-foreground">
               {entry?.dataKey === 'requests' 
                 ...
                 : entry.dataKey === 'USD' ? `$${entry.value.toLocaleString()}` : `${entry.value.toLocaleString()} KHR`
               }
             </span>
           </div>
         ))}
       ...
       <div className="h-80">
         <ResponsiveContainer width="100%" height="100%">
           ...
           >
             ...
             />
             <YAxis 
               yAxisId="left" // Define left axis
               stroke="hsl(var(--muted-foreground))"
               fontSize={12}
               tickFormatter={(value) => formatCurrency(value, 'USD')}
             />
             {/* --- ADD THIS Y-AXIS FOR KHR --- */}
             <YAxis 
               yAxisId="right" 
               orientation="right" 
               stroke="hsl(var(--muted-foreground))"
               fontSize={12}
               tickFormatter={(value) => formatCurrency(value, 'KHR')}
             />
             <Tooltip content={<CustomTooltip />} />
             <Legend />
             <Bar 
               yAxisId="left" // Assign bar to left axis
               dataKey="USD" 
             ...
             />
             <Bar 
               yAxisId="right" // Assign bar to right axis
               dataKey="KHR" 
